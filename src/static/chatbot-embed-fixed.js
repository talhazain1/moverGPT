/**
 * Chatbot Embed Script (Fixed Version)
...
(function() {
  ...
  async function initSession(config) {
    try {
      if (!sessionId) {
        ...
        script.src = `${config.serverUrl}/api/chatbot/jsonp/session?callback=${jsonpCallback}&apiKey=${encodeURIComponent(config.apiKey)}&chatbotId=${encodeURIComponent(config.chatbotId)}&companyId=${encodeURIComponent(config.companyId)}`;        
  ...
  async function sendMessage() {
    ...
    try {
      ...
      const response = await fetch(`${defaultConfig.serverUrl}/api/chatbot/session/${sessionId}/message`, {
        method: 'POST',
        headers: {
  ...
  async function getBotResponse(userMessage) {
    try {
      ...
      // Get bot response
      const response = await fetch(`${defaultConfig.serverUrl}/api/chatbot/inference`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${accessToken || ''}`
        ...
        body: JSON.stringify({
          company_id: defaultConfig.companyId,
          query: userMessage,
          session_id: sessionId,
          website_url: defaultConfig.websiteUrl
        })
      });
      ...
      if (!response.ok) {
        console.error("Error getting bot response. Status:", response.status);
