...
(function() {
  ...
  let accessToken = localStorage.getItem('chatbot_access_token');

  // Initialize the widget
  function initChatWidget(config) {
    const mergedConfig = { ...defaultConfig, ...config };
    defaultConfig.accessToken = mergedConfig.accessToken;
  ...
  async function initSession(config) {
    try {
      if (!sessionId) {
        ...
        const response = await fetch(`${config.serverUrl}/api/chatbot/session`, {
          method: 'POST',
          headers: {
  ...
  async function fetchMessages(config) {
    try {
      const response = await fetch(`${config.serverUrl}/api/chatbot/session/${sessionId}/messages`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${accessToken || ''}`
        }
  ...
  async function getBotResponse(userMessage) {
    try {
      ...
      if (response.ok) {
        ...
        console.error("Error getting bot response:", data.error);
  ...
  window.initChatbotWidget = initChatWidget;
})(); 