...
def direct_inference():
    ...
    try:
        ...
        current_app.logger.info(f"Direct inference request: key={api_key[:8] if api_key else 'None'}..., company={company_id}, message={message}")
        ...
        if not api_key_obj:
            try:
                ...
                if any_key:
                    ...
                    if any_key.status == 'active':
                        api_key_obj = any_key
                        current_app.logger.info("API key is active in api_keys model")
                    else:
                        current_app.logger.warning(f"API key found but status is not active: {any_key.status}")
                else:
                    current_app.logger.warning("API key not found in api_keys model")
            except Exception as e:
        ...
        if not session_id or not session_id.startswith('local_session_'):
            ...
            try:
                ...
                # Make sure to use the correct parameters expected by run_inference_openai
                response_text = run_inference_openai(
                    chatbot_config=chatbot_config,
                    query=message,
                    history=[],  # No history for direct API
                    session_id=session_id,
                    top_n=3  # Default number of relevant FAQs to retrieve
                )
